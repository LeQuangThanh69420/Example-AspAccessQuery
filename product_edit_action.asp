...
<%
	...
	sql="select * From [0204466_Product_97] where pname='" & pname &"' and pid<>" & pid 
	rs.open sql, conn 
	if (not rs.eof) then
		Session("product_edit_error")="Tên sản phẩm: " & pname &" đã có rồi!"
		Response.Redirect("product_edit.asp")	
    else 
		sql = "update [0204466_Product_97] set pname='" & pname & "',pdesc='" & pdesc & "',pimage ='" & pimage & "',pprice = " & pprice & ",pquantity = " & pquantity & ",cid = " & cid & ",sid = " & sid & ",sizeid = " & sizeid & ",pstatus = " & pstatus & " where pid = " & pid 
		'Response.write(sql)
		conn.execute sql 
		conn.close
